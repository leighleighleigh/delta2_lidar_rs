use crate::protocol::{PartialFrame, MeasurementFrame};
...
impl Lidar {
    ...
    pub fn alive(&self) -> bool {
        ...
        match self.rx.is_some() && self.worker_handle.is_some() {
            true => {
                !self.worker_handle.as_ref().unwrap().is_finished()
            },
            false => false,
        }
    }
    ...
    pub fn open(&mut self, path: String) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.worker_handle = Some(thread::Builder::new().name("lidar_decode_thread".to_string()).spawn(move || {
            let port_name = path.as_str();
            let port_builder = serialport::new(port_name, 115200).timeout(Duration::from_millis(20));
                ...
                loop {
                    match serial.read(serial_temp_buf.as_mut_slice()) {
                        Ok(t) => {
                            ...
                            serial_data_buffer= serial_data_buffer.as_mut_slice()[..n].to_vec();
                                        ...
                                        Some(code) => {
                                            match code {
                                                105 => {
                                                    ...
                                                    debug!("Frame full. t = {}, frame = {}", t, new_frame.data.len());
                                                },
                                                _ => {}
                                            }
                                        },
